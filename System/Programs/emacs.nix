{
  ...
  overrides = final: prev: {
    ...
    #       hash = "sha256-5NDZZvlZZ+3fVij0FU063qGxmG1+qo4uZ6FnsjAW+bQ=";
    #     })
    #   ];
    # };
  ...
  makeWrapperArgs = [
    "--set"
    "DICPATH"
    (lib.makeSearchPath "share/hunspell" hunspellDicts)
    "--prefix"
    "PATH"
    ":"
...
in {
  config = lib.mkIf cfg.enable {
    services.emacs = {
      enable = true;
      package = finalPackage; # replace with emacs-gtk, or a version provided by the community overlay if desired.
    };
  };
}
